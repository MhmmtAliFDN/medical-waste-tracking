        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{route('authorized-officer.add')}}" class="btn btn-rounded btn-primary">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-responsive-sm text-dark">
                            ...
                            <tbody>
                            @foreach($authorizedOfficers as $authorizedOfficer)
                                <tr itemid="{{$authorizedOfficer->id}}">
                                    <td>{{$authorizedOfficer->id}}</td>
                                    <td>{{$authorizedOfficer->user->name}}</td>
                                    <td>{{ date('j.m.Y', strtotime($authorizedOfficer->created_at)) }}
                                    </td>
                                    <td>
                                        <span>
                                            <a type="button" href="{{route('authorized-officer.edit', ['id' => $authorizedOfficer->id])}}"
        ...
        $(document).ready(function () {
            $(document).on('click', '#btn-delete-entity', function (e) {
                ...
                if (confirm('Bu ögeyi silmek istediğinize emin misiniz?')) {
                    $.ajax({
                        url: '{{route('authorized-officer.destroy')}}',
                        type: 'DELETE',
                        data: {id: entityId},
